Cypress.Commands.add('getIframeBody', (attribute) => {
  ...
  describe('dcTrack front-end testing ', () => {
    ...
    it('Add an Item', () => {
      cy.wait(4000)
      cy.getIframeBody('id="circuits_iframe"').find('#circuitList') .find('[title="Add a Circuit"]').click()
      cy.wait(8000)
      cy.getIframeBody('id="circuits_iframe"').find('#circuit-trace-item-list')
        .contains('div', '1A1').click()
      cy.wait(12000)
      cy.getIframeBody('id="circuits_iframe"').find('#dataPortsGrid')
        .find('.ui-grid-canvas').eq(1).find('.ui-grid-row').eq(1).find('[role="rowheader"]').click() // A0004
      cy.wait(10000)
      cy.getIframeBody('id="circuits_iframe"').find('#itemsListGrid')
        .contains('div', 'DATA-01-01').click()  // DATA-01-01
      cy.wait(4000)
      cy.getIframeBody('id="circuits_iframe"').find('#dataPortsGrid')
        .find('.ui-grid-canvas').eq(1).find('.ui-grid-row').eq(0).find('[role="rowheader"').click() //P01
      // here will create a circuit with trace:  "Data AF1:A0004"
      cy.wait(2000)
      cy.getIframeBody('id="circuits_iframe"').find('#circuitTraceTb').find('[title="Save"]').click()
      cy.wait(8000)
    })

    it('Connect it and verify the messages', () => {
      cy.getIframeBody('id="circuits_iframe"').find('#circuitTraceTb').find('[title="Actions"]').click()
      cy.getIframeBody('id="circuits_iframe"').find('#circuitTraceTb').find('#dropdown-ACTION_TB > li').as('dropdown')
      ...
      cy.wait(4000)
      cy.getIframeBody('id="circuits_iframe"').find('#idItemRequestTextArea').should(($t) => {
    ...
    it('Disconnect the circuit', () => {
      cy.getIframeBody('id="circuits_iframe"').find('#inner-scroller > ul').find('li').eq(1)
        .find('button').click()
      cy.wait(2000)
      cy.getIframeBody('id="circuits_iframe"')
        .contains('div', 'AF1:A0004 - 1A1:A0004 ~ DATA-01-01:P01').click()
      cy.wait(2000)
