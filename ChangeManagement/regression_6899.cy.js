Cypress.Commands.add('getIframeBody', (attribute) => {
  ...
  describe('dcTrack front-end testing ', () => {
    ...
    it('Prepare Move item request for AMSTAD_T3 and check the origin cabinets length', () => {
      cy.wait(10000)
      cy.getIframeBody('id="assets_iframe"').find('#filter_tiName').type('AMSTAD_T3{enter}')
      cy.wait(4000)
      cy.getIframeBody('id="assets_iframe"').find('#myGrid')
        .contains('div', 'AMSTAD_T3').eq(0).click()
      cy.wait(2000)
      ...
      cy.wait(4000)
      cy.getIframeBody('id="assets_iframe"').find('#dct-capacity-modal-content').find('.dct-search-results')
        .find('[role="presentation"]').eq(1)
        .find('.ui-grid-canvas').find('.ui-grid-row').as('rows')
      cy.get('@rows').should('have.length', 14)
    })

    it('Change a connection and verify if the cabinets length changes', () => {
      cy.getIframeBody('id="assets_iframe"').find('#idPowerPortGridBlock')
        .find('.ui-grid-canvas').find('.ui-grid-row').eq(0).contains('div', 'To Rack PDU').dblclick()
      // ERROR: I can't take action on the dropdown
      cy.getIframeBody('id="assets_iframe"').find('#idPowerPortGridBlock')
        .find('.ui-grid-canvas').find('.ui-grid-row').eq(0).contains('div', 'To Rack PDU').parent()
        .find('div').eq(1).children(':nth-child(2)').click()
