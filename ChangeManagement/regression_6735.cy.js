  ...
  describe('dcTrack front-end testing ', () => {
    ...
    it('Install two items, one can install successfully, the other should failed', () => {
      ...
      cy.getIframeBody('id="assets_iframe"')
        .contains('div', '401-XIV-1').eq(0).click({ ctrlKey: true })
      cy.getIframeBody('id="assets_iframe"')
        .contains('div', '6M').eq(0).click({ ctrlKey: true })
      cy.wait(2000)
    ...
    it('Verify 6M success and 401-XIV-1 failed', () => {
      ...
      const failedMsg = "Install Item: 401-XIV-1. Item 401-XIV-1 cannot be Installed because its Cabinet is not in the Installed status and is not included in this work order."
    ...
    it('Make 6M back to planned status', () => {
      cy.getIframeBody('id="assets_iframe"')
        .contains('div', '6M').eq(0).dblclick()
      cy.wait(4000)
      cy.getIframeBody('id="assets_iframe"').find('#itemDetail').find('[title="Edit"]').click()
      cy.wait(2000)
      cy.getIframeBody('id="assets_iframe"').find('[uib-tooltip="Installed"]').click()
    ...
    it('Cancel the second request', () => {
      ...
      cy.wait(4000)
      cy.getIframeBody('id="change_iframe"').find('.ui-grid-canvas > div').eq(1).click()
