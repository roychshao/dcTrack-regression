...
describe("dcTrack front-end testing ", () => {
  ...
  it("Take a snapshot and compare", () => {
    cy.getIframeBody('id="floormap_iframe"')
      .find("#container3D")
      .compareSnapshot({ name: "cabinet-moveon", testThreshold: 0.05 });
  });

  it("Mouse moveoff the cabinet", () => {
    cy.getIframeBody('id="floormap_iframe"')
      .find("#container3D")
      .should("be.visible")
      .then(($canvas) => {
        ...
        cy.wrap(canvas).realMouseDown({ position: "center" });
        cy.wrap(canvas).realMouseUp();
        cy.wrap(canvas).realMouseMove(0, 0, { position: "topLeft" });
        cy.wait(1000);
      });
  });
  ...
  it("Take a snapshot and compare", () => {
    cy.getIframeBody('id="floormap_iframe"')
      .find("#container3D")
      .compareSnapshot({ name: "cabinet-moveoff", testThreshold: 0.05 });
  });
  ...
  it("Moveon and compare snapshot", () => {
    cy.wait(2000);
    cy.getIframeBody('id="floormap_iframe"')
      .find("#container3D")
      .should("be.visible")
    ...
    cy.getIframeBody('id="floormap_iframe"')
      .find("#container3D")
      .compareSnapshot({ name: "isolated-moveon", testThreshold: 0.05 });
  });

  it("Moveoff and compare snapshot", () => {
    cy.wait(2000);
    cy.getIframeBody('id="floormap_iframe"')
      .find("#container3D")
      .should("be.visible")
    ...
    cy.getIframeBody('id="floormap_iframe"')
      .find("#container3D")
      .compareSnapshot({ name: "isolated-moveoff", testThreshold: 0.05 });
  });
});
