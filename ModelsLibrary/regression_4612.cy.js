Cypress.Commands.add('getIframeBody', (attribute) => {
  ...
  describe('dcTrack front-end testing ', () => {
    ...
    it('Try to delete a make', () => {
      cy.wait(4000)
      cy.getIframeBody('id="models_iframe"').find('#modellist')
        .find('[title="Makes"]').click()
      cy.getIframeBody('id="models_iframe"').find('#makes-list')
        .contains('div', '2BM').click()
      cy.getIframeBody('id="models_iframe"').find('#makesList')
        .find('[title="Delete"]').click()
      cy.getIframeBody('id="models_iframe"').find('.dct-msg-body')
    ...
    it('Verify if 2BM is still visible', () => {
      cy.getIframeBody('id="models_iframe"').find('#filter_make').type('2BM{enter}')
      cy.wait(4000)
