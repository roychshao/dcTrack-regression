Cypress.Commands.add('getIframeBody', (attribute) => {
  ...
  describe('dcTrack front-end testing ', () => {
    ...
    it('Find 00YJ783', () => {
      ...
      cy.wait(2000)
      cy.getIframeBody('id="models_iframe"').find('#powerPortsGrid')
        .find('.ui-grid-canvas').eq(3).children().eq(1).find('[title="20"]').parent()
        .click().clear()
      // To make it savable
    ...
    it('Verify error message and cancel this modification', () => {
      cy.wait(8000)
      const errMsg = "RPDU Outlet [ Outlet 1 ] Fuse and Fuse Amps should both be filled or left empty."
      cy.getIframeBody('id="models_iframe"').find('.modal-content').contains('p', errMsg)
