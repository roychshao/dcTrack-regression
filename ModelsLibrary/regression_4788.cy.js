Cypress.Commands.add('getIframeBody', (attribute) => {
  ...
  describe('dcTrack front-end testing ', () => {
    ...
    it('Edit 0000000111111, save and inspect data', () => {
      ...
      cy.getIframeBody('id="models_iframe"') .find('#tiMounting-selectized').parent().click()
      cy.getIframeBody('id="models_iframe"') .find('[data-value="Non-Rackable"]').click() 
      cy.getIframeBody('id="models_iframe"').find('#modeldetail')
        .find('[title="Save"]').click()
      cy.wait(10000)
      cy.getIframeBody('id="models_iframe"') .find('#tiMounting-selectized').parent()
    ...
    it('Rollback', () => {
      cy.wait(4000)
      cy.getIframeBody('id="models_iframe"') .find('#tiMounting-selectized').parent().click()
      cy.getIframeBody('id="models_iframe"') .find('[data-value="Rackable"]').click() 
      cy.getIframeBody('id="models_iframe"').find('#modeldetail')
        .find('[title="Save"]').click()
    })
    ...
    it('Edit 001 ENC-02, save and inspect data', () => {
      cy.getIframeBody('id="models_iframe"')
        .contains('div', '001 ENC-02').dblclick()
      cy.wait(8000)
      cy.getIframeBody('id="models_iframe"').find('#subtab-heading-1').find('#btnRemoveTab').click()
      cy.getIframeBody('id="models_iframe"').find('#modeldetail')
        .find('[title="Edit"]').click()
      cy.getIframeBody('id="models_iframe"') .find('#tiMounting-selectized').parent().click()
      cy.getIframeBody('id="models_iframe"') .find('[data-value="Non-Rackable"]').click() 
