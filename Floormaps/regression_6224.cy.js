...
describe("dcTrack front-end testing ", () => {
  ...
  it("Click item and compare snapshot", () => {
    cy.wait(2000);
    cy.getIframeBody('id="floormap_iframe"')
      .find("#container3D")
      .should("be.visible")
    ...
    cy.getIframeBody('id="floormap_iframe"')
      .find("#container3D")
      .compareSnapshot({ name: "cabinet-face", testThreshold: 0.05 });
  });

  it("Change view and click on the back of item", () => {
    cy.wait(4000);
    cy.getIframeBody('id="floormap_iframe"')
      .find("#container3D")
      .should("be.visible")
      .then(($canvas) => {
        ...
        cy.wrap(canvas).realMouseDown({ position: "center" });
        cy.wrap(canvas).realMouseMove(140, 0, { position: "center" });
        cy.wrap(canvas).realMouseUp();
        cy.wait(2000);
        cy.wrap(canvas).realMouseMove(0, 0, { position: "center" });
        cy.wrap(canvas).realMouseDown({ position: "center" });
        cy.wrap(canvas).realMouseUp();
      });
  });
  ...
  it("Take a snapshot and compare", () => {
    ...
    cy.getIframeBody('id="floormap_iframe"')
      .find("#container3D")
      .compareSnapshot({ name: "cabinet-back", testThreshold: 0.05 });
  });
});
