...
describe("dcTrack front-end testing ", () => {
  ...
  it("Make show content always", () => {
    cy.wait(8000);
    cy.getIframeBody('id="floormap_iframe"')
      .find("#floormap")
      .find('[title="Click to open Configuration dialog"]')
      .click();
    cy.getIframeBody('id="floormap_iframe"')
      .find('a[title="Cabinets"]')
      .click();
    cy.getIframeBody('id="floormap_iframe"')
      .contains("label", "Always")
      .parent()
      .find("input")
      .click();
    cy.getIframeBody('id="floormap_iframe"')
  ...
  it("Click on 1B-RPDU-R and compare shanshot", () => {
    cy.getIframeBody('id="floormap_iframe"')
  ...
  it("Enable isolated mode", () => {
    ...
    cy.getIframeBody('id="floormap_iframe"')
      ...
      .then(($canvas) => {
        const canvas = $canvas[0];
        cy.wrap(canvas).realMouseMove(147, 223, { position: "center" }).click();
        cy.wait(2000);
        // it have to move again
        cy.wrap(canvas).realMouseMove(0, 250, { position: "center" });
        cy.wrap(canvas).realMouseDown();
        cy.wrap(canvas).realMouseUp();
      });
  });

  it("Click on an item and compare snapshot", () => {
    cy.wait(2000);
    cy.getIframeBody('id="floormap_iframe"')
      .find("#container3D")
      .should("be.visible")
      .then(($canvas) => {
        const canvas = $canvas[0];
        cy.wrap(canvas).realMouseMove(-32, 44, { position: "center" });
        cy.wrap(canvas).realMouseDown();
        cy.wrap(canvas).realMouseUp();
      });
    cy.wait(6000);
